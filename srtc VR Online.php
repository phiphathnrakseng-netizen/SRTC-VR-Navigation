    ...
    <style>
        ...
        * {
        ...
        #aframe-scene-container {
            ...
            background-color: #f0f0f0; /* สีพื้นหลัง A-Frame */
            border-radius: 0.75rem;
            overflow: hidden;
        }
    </style>
...
<div id="main-content" class="flex flex-col flex-grow">
    <header class="bg-white shadow-lg sticky top-0 z-20 border-b border-gray-100">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 sm:py-4 flex justify-between items-center">
            ...
            <div class="flex items-center space-x-3">
                <img src="https://www.srtc.ac.th/2020/images/images.jpg" alt="SRTC Logo" class="h-9 w-9 sm:h-10 sm:w-10">
            ...
            <nav class="space-x-2 sm:space-x-4 flex items-center">
                <a href="#home" onclick="showSection('home-section')" class="text-gray-700 hover:text-indigo-600 transition duration-150 hidden lg:inline-block font-semibold" id="nav-home">หน้าแรก</a>
                
                <a href="#map" onclick="showSection('map-section')" class="text-gray-700 hover:text-indigo-600 transition duration-150 hidden lg:inline-block font-semibold" id="nav-map">แผนผัง</a>
                
                <button id="pwa-install-button" onclick="promptPWAInstall()" class="hidden bg-purple-500 hover:bg-purple-600 text-white font-semibold py-2 px-3 sm:px-4 rounded-full shadow-md flex items-center space-x-2 transform transition duration-200 hover:scale-105">
    ...
    <div id="home-section" class="section-content flex-grow flex flex-col">
        <section class="relative flex-grow flex items-center justify-center text-center overflow-hidden h-[75vh] lg:h-[80vh] min-h-[500px]">
            <img 
            ...
            <div class="relative z-10 max-w-2xl p-6">
                <h2 class="text-4xl sm:text-5xl lg:text-6xl font-extrabold text-white mb-4 leading-snug">ระบบนำทางห้องเรียนอัจฉริยะที่ทันสมัย</h2>
                <p class="text-lg sm:text-xl text-indigo-200 mb-8">ด้วยระบบเทคโนโลยีเสมือน AR/VR ที่สมจริง</p>
                
                <div class="flex rounded-full shadow-2xl overflow-hidden max-w-md mx-auto">
                    <input type="text" placeholder="ค้นหาห้องเรียน เช่น 201, ห้องปฏิบัติการคอมพิวเตอร์" class="w-full py-3 px-6 text-gray-800 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition duration-150 text-base sm:text-lg" aria-label="Search">
                ...
                <button onclick="showSection('map-section')" class="mt-8 bg-pink-500 hover:bg-pink-600 text-white font-bold py-3 px-6 rounded-full shadow-lg transition duration-200 transform hover:scale-105 flex items-center justify-center mx-auto space-x-2">
    ...
    <div id="map-section" class="section-content hidden flex-grow p-4 sm:p-8">
        <div class="max-w-7xl mx-auto">
            <h2 class="text-3xl sm:text-4xl font-extrabold text-gray-900 mb-6 border-b pb-2 flex items-center">
            ...
            <div id="building-list" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                </div>
            
            <div class="mt-10 p-6 bg-yellow-100 border-l-4 border-yellow-500 rounded-lg">
...
<div id="building-detail-modal" class="hidden fixed inset-0 bg-gray-900 bg-opacity-75 z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto p-6 sm:p-8 transform transition-all duration-300">
        <div class="flex justify-between items-start mb-4">
            <h3 id="building-title" class="text-2xl font-bold text-indigo-700 flex items-center">
                <i data-lucide="building-2" class="w-6 h-6 mr-2"></i>
                รายละเอียดอาคาร
            </h3>
            <button onclick="hideModal('building-detail-modal'); stopAframeScene();" class="text-gray-400 hover:text-gray-600 transition">
                <i data-lucide="x" class="w-6 h-6"></i>
            </button>
        </div>
        
        <div class="flex space-x-4 mb-6 border-b border-gray-200">
            <button id="tab-detail" onclick="showBuildingTab('detail')" class="py-2 px-4 text-lg font-semibold text-white bg-indigo-600 rounded-t-lg transition duration-150 flex items-center">
                <i data-lucide="info" class="w-5 h-5 mr-2"></i> ข้อมูลอาคาร
            </button>
            <button id="tab-3d" onclick="showBuildingTab('3d')" class="py-2 px-4 text-lg font-semibold text-gray-600 hover:text-indigo-600 transition duration-150 flex items-center">
                <i data-lucide="cube" class="w-5 h-5 mr-2"></i> ดูแบบ 3D/VR
            </button>
        </div>

        <div id="building-tab-detail" class="tab-content space-y-4">
            <div id="building-image-container" class="rounded-xl overflow-hidden shadow-lg h-60 bg-gray-200 flex items-center justify-center">
                <img id="building-image" src="" alt="รูปภาพอาคาร" class="object-cover w-full h-full">
            </div>
            <p id="building-description" class="text-gray-700"></p>
            <div class="grid grid-cols-2 gap-4 text-gray-700">
                ...
                <p><strong>ลิงก์แผนที่:</strong> <a id="building-map-link" href="#" target="_blank" class="text-indigo-600 hover:underline">นำทางด้วย Google Maps</a></p>
            </div>
        </div>
        ...
        <div id="building-tab-3d" class="tab-content hidden">
            ...
            <div class="mt-4 p-4 bg-blue-100 border-l-4 border-blue-500 rounded-lg">
                <p class="text-blue-800 text-sm flex items-center">
                    <i data-lucide="vr-glasses" class="w-5 h-5 mr-2"></i>
                    **เคล็ดลับ:** หากใช้มือถือ สามารถกดปุ่ม VR มุมขวาล่างในฉาก 3D เพื่อเข้าสู่โหมด VR/Cardboard
                </p>
            </div>
        </div>

        <button onclick="hideModal('building-detail-modal'); stopAframeScene();" class="mt-8 bg-red-500 hover:bg-red-600 text-white font-bold py-2 px-4 rounded-full shadow-md transition duration-150 w-full flex items-center justify-center space-x-2">
...
<div id="vr-container" class="hidden fixed inset-0 z-50 bg-black flex flex-col items-center justify-center w-screen h-screen">
    ...
    <div class="relative z-10 w-full h-full p-4 flex flex-col justify-between items-center pointer-events-none">
        <div id="ar-message" class="bg-yellow-400 text-gray-900 font-bold p-3 rounded-xl shadow-xl text-center transition-opacity duration-500 opacity-0 pointer-events-auto text-sm sm:text-base">
        ...
        <div class="flex flex-col items-center mt-auto mb-16 text-white text-center">
            ...
            <p id="ar-destination" class="text-xl sm:text-3xl font-extrabold drop-shadow-lg bg-black/50 p-2 rounded-lg">มุ่งหน้าไปยัง: ห้อง 305 (ระยะทาง 15 ม.)</p>
        </div>
    </div>
    ...
    <div id="error-message" class="hidden absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-red-800 text-white p-6 rounded-xl shadow-2xl z-30 pointer-events-auto max-w-sm text-center">
...
<script>
    ...
    window.showSection = function(sectionId) {
        ...
        if (sectionId === 'home-section') {
            ...
            document.getElementById('nav-map').classList.add('text-indigo-600');
        }
    }
    ...
    window.showBuildingDetail = function(buildingId) {
        ...
        document.getElementById('building-title').innerHTML = `<i data-lucide="building-2" class="w-6 h-6 mr-2"></i> ${building.name} (${building.short_name})`;
    ...
    window.showBuildingTab = function(tabName, building = null) {
        ...
        document.querySelectorAll('button[id^="tab-"]').forEach(btn => {
            ...
            btn.classList.add('text-gray-600', 'hover:text-indigo-600', 'bg-white');
        });
        ...
        document.getElementById(`tab-${tabName}`).classList.remove('text-gray-600', 'hover:text-indigo-600', 'bg-white');
        ...
        buildingListContainer.innerHTML = buildingsData.map(building => `
            <div class="bg-white rounded-xl shadow-lg hover:shadow-xl transition duration-300 overflow-hidden border border-gray-100 flex flex-col">
                ...
                <div class="p-5 flex flex-col flex-grow">
                    <h4 class="text-xl font-bold text-indigo-700 mb-1">${building.name}</h4>
                    ...
                    <button 
                        ...
                        class="mt-auto bg-indigo-500 hover:bg-indigo-600 text-white font-semibold py-2 rounded-lg shadow-md transition duration-150 flex items-center justify-center space-x-2 transform hover:scale-[1.02]"
                    >
    ...
    async function startCamera() {
        ...
        arMessage.classList.remove('hidden', 'opacity-0');
    ...
    function stopCamera() {
        ...
        arMessage.classList.remove('bg-green-500', 'bg-yellow-400');
    ...
    window.toggleVRMode = function() {
        ...
        if (isVRActive) {
            ...
            mapSection.classList.add('hidden'); // ซ่อน Map ด้วย
            vrContainer.classList.remove('hidden');
            mainContent.classList.add('hidden'); // ซ่อน Main Content ทั้งหมด
            startCamera();
        } else {
            stopCamera();
            mainContent.classList.remove('hidden'); // แสดง Main Content
            homeSection.classList.remove('hidden'); // กลับไปหน้า Home
